...
@RequestMapping("/board") // 작성 시 헷갈렸던 부분. return부분을 작성할 때 html위치.
public class BoardController {
    ...
    @PostMapping("/save")
    public String save(@ModelAttribute BoardDTO boardDTO) throws IOException { //추가
        System.out.println("boardDTO=" + boardDTO);
        boardService.save(boardDTO);
        return "/yoonseo/yoonseoBoard"; // 전에 만들었던거에서는 index였지만 지금은 index를 yoonseoBoard로 했기에 이걸로 대체
    }
    //글 목록부분
    @GetMapping("/")
    ...
    public String paging(@PageableDefault(page = 1) Pageable pageable, Model model) {
        ...
        return "/yoonseo/paging"; //list와 마찬가지로!
